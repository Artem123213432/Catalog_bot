...
async def get_phone(message: Message, state: FSMContext):
    ...
    text = f"<b>Проверьте данные заказа:</b>\nИмя: {data['name']}\nАдрес: {data['address']}\nТелефон: {data['phone']}\n\nПодтвердить заказ? (да/нет)"
...
async def confirm_order(message: Message, state: FSMContext):
    ...
    # Формируем данные для Django API
    order_data_for_api = {
        "client": {
            "name": data['name'],
            "phone": data['phone'],
        ...
        "items": [
            {"product_id": item.get("id"), "quantity": item.get("quantity", 1)} for item in cart
        ]
    }
    
    try:
        # Сохраняем в Excel
        save_order(order_data_for_api)
        ...
        asyncio.create_task(check_payment_status(message, state, payment['id']))
        
        # Отправляем заказ в Django
        asyncio.create_task(send_order_to_django(order_data_for_api))
...
async def check_payment_status(message: Message, state: FSMContext, payment_id: str):
    ...
    for _ in range(30):  # Проверяем в течение 5 минут
